...
if [ -z "$CURRENT_CONFIG" ]; then
    echo "❌ Error: Failed to get realm configuration"
    exit 1
fi

print_success "Retrieved current realm settings"
echo ""
...
echo "  Access Token Lifespan: $CURRENT_ACCESS_TOKEN_LIFESPAN seconds"
echo "  SSO Session Idle: $CURRENT_SSO_SESSION_IDLE seconds"
echo "  SSO Session Max: $CURRENT_SSO_SESSION_MAX seconds"
echo "  Offline Session Idle: $CURRENT_OFFLINE_SESSION_IDLE seconds"
echo "  Refresh Token Max Reuse: $CURRENT_REFRESH_TOKEN_MAX_REUSE"
echo ""
...
echo "New Settings:"
echo "  Access Token Lifespan: $ACCESS_TOKEN_LIFESPAN seconds (4 hours)"
echo "  SSO Session Idle: $SSO_SESSION_IDLE seconds (8 hours)"
echo "  SSO Session Max: $SSO_SESSION_MAX seconds (12 hours)"
echo "  Offline Session Idle: $OFFLINE_SESSION_IDLE seconds (30 days)"
echo "  Refresh Token Max Reuse: $REFRESH_TOKEN_MAX_REUSE (unlimited)"
echo ""
...
if [ "$HTTP_CODE" != "204" ]; then
    print_fail "Error: Failed to update realm settings (HTTP $HTTP_CODE)"
    echo "Response: ${UPDATE_RESPONSE%???}"
    exit 1
fi

print_success "Successfully updated realm token lifespans!"
echo ""
