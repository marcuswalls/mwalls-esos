...
if [ -f "/tmp/dockerhost-root-ca.crt" ]; then
    ...
    sudo keytool -import -trustcacerts -keystore $JAVA_HOME/lib/security/cacerts -storepass changeit -alias dockerhost-ca -file /tmp/dockerhost-root-ca.crt -noprompt || true
