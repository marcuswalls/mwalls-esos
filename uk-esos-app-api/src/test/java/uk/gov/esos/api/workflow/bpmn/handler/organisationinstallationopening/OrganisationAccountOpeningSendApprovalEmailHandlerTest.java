...
class OrganisationAccountOpeningSendApprovalEmailHandlerTest {

    @InjectMocks
    private OrganisationAccountOpeningSendApprovalEmailHandler sendApprovalEmailHandler;

    @Mock
    private OrganisationAccountOpeningSendApprovalEmailService sendApprovalEmailService;
    ...
    void execute() {
        ...
        sendApprovalEmailHandler.execute(execution);

        verify(execution, times(1)).getVariable(BpmnProcessConstants.REQUEST_ID);
        verify(sendApprovalEmailService, times(1)).sendEmail(requestId);
    }
}