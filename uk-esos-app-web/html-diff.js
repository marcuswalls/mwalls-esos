...
calculate_operations = function (before_tokens, after_tokens) {
  ...
  for (index = i = 0, len = matches.length; i < len; index = ++i) {
    ...
    action_up_to_match_positions =
      action_map[[match_starts_at_current_position_in_before, match_starts_at_current_position_in_after].toString()];
...
wrap = function (tag, content, klass = '') {
  ...
  while (true) {
    ...
    if (non_tags.length !== 0) {
      var tagWithClass = klass.length > 0 ? `${tag} class="${klass}"` : `${tag}`;
      rendering += `<${tagWithClass}>${non_tags.join('')}</${tag}>`;
    }
    if (position >= length) {
      break;
    }
