...
export const significantEnergyConsumptionExistsFormProvider: Provider = {
  provide: TASK_FORM,
  ...
  useFactory: (fb: UntypedFormBuilder, store: RequestTaskStore, subtask: CompliancePeriod) => {
    ...
    return fb.group({
      significantEnergyConsumptionExists: [
        compliancePeriod?.firstCompliancePeriodDetails?.significantEnergyConsumptionExists,
        GovukValidators.required('Select yes if areas of significant consuption were identified'),
      ],
    });
  },
};
