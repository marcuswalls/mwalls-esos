...
#This script updates the uk-esos realm in order to configure the password policy based on length(minimun 12 characters, max 128 and 100000 hashIterations).
...
#Configures the uk-esos realm with the desired password policy
CONFIG_UK_ESOS_REALM_PASSWORD_POLICY=$(curl -s -L -X PUT "$UPDATE_REALM_URL$UK_ESOS_REALM_NAME" \
...
if [ -z "$CONFIG_UK_ESOS_REALM_PASSWORD_POLICY" ]
then
	...
	echo " $CONFIG_UK_ESOS_REALM_PASSWORD_POLICY"
	exit;
fi
